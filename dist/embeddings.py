...
def get_model(
    ...
    **kwargs,
    ...
    if optimization and not use_cpu:
...
def generate_embeddings_llm(
    ...
    ```
    """
    tokenizer = get_tokenizer(model_name, token, cache_dir=cache_dir)
    model = get_model(model_name, token, optimization=True, use_cpu=use_cpu, cache_dir=cache_dir)  # type: ignore
    model.eval()
    for i, event in enumerate(events):
        tokenized_full_text = tokenizer.encode(event)
